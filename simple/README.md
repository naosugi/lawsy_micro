...
1. **Web検索実行**
   - 元のクエリをLLMでWeb検索用に最適化
   - 最適化されたクエリでWeb検索実行（フリードメイン・ドメイン指定）

2. **クエリ拡張**
   - Web検索（タイトルとスニペット）をもとに、LLMが関連サブクエリを生成（例：「労働基準法第19条の解雇予告制度について」）

3. **法令検索実行**
   - 元のクエリと拡張クエリでドキュメント検索実行（OpenSearch/Vertex AI Search/FAISSなど）

4. **検索結果統合**
   - Web検索・法令検索の結果を結合し、リランキング
   - 重複排除（オプション）
